...
sub acl_add {
    ...
    my $command = qq{acl-add $endpoint --identityusername=${email} --perm $readwrite } ;
    my $result
        = _globus_action( $command, $self->{username}, $self->{key_path} ) ;
    ...
    my %result = 
        map {
            ...
            %$hash = map {
                ...
                split m{\n} ;
            my $id = $hash->{display_name} ne 'n/a' ?
                $hash->{display_name} : $hash->{legacy_name} ; 
            $id => $hash ;
            }
        split m{\n\n} , $result ;
    return wantarray ? %result : \%result ;
